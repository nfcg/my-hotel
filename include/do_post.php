...
if (in_array($do, $do_admin) and $_SESSION["SITE"] != 'true') { echo json_encode("do $NOT_ALLOWED"); exit(); }
         
switch ($do) {
  case "booking":
    unset($error);
    
    if (isset($_POST["double_bed"])) { $double_bed = SQLite3::escapeString($_POST["double_bed"]); } else { $double_bed = "false"; }
    if (isset($_POST["canc_pol"])) { $cancel_policy = SQLite3::escapeString($_POST["canc_pol"]); } else { $cancel_policy = "off"; }
...
try {
    if (empty($check_in)) {throw new Exception($INVALID_CHECKIN_DATE);}
    if (empty($check_out)) {throw new Exception($INVALID_CHECKOUT_DATE);}
    if (empty($room_type)) {throw new Exception($INVALID_NUMBER_GUESTS);}
    if (empty($first_name)) {throw new Exception($INVALID_NAME);}
    if (empty($last_name)) {throw new Exception($INVALID_SURNAME);}
    if (empty($phone)) {throw new Exception($INVALID_PHONE);}
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {throw new Exception($INVALID_INVALID_EMAIL);}
    if (empty($cc_type)) {throw new Exception($INVALID_CARD_TYPE);}
    if (empty($cc_name)) {throw new Exception($INVALID_CARDHOLDER_NAME);}
    if ($cancel_policy != "on") {throw new Exception($MANDATORY_CANCELLATION_POLICY);}
    if ($check_out <= $check_in) {throw new Exception($INVALID_CHECKIN_DATE . ' - ' . $INVALID_CHECKOUT_DATE);}
    $card_cvc_t = CreditCard::validCvc($cc_cvc, $cc_type);
    if ($card_cvc_t !== true) {throw new Exception($INVALID_CARD_CVC);}
    $card_n_t = CreditCard::validDate($cc_expire_y, $cc_expire_m);
    if ($card_n_t !== true) {throw new Exception($INVALID_CARD_EXPIRATION_DATE);}
    $card = CreditCard::validCreditCard($cc_number, $cc_type);
    if ($card["valid"] !== true) {throw new Exception($INVALID_CARD_NUMBER);}

    $check_in_dif       = date_create($check_in);
    $check_out_dif      = date_create("$check_out");

    $check_out          = date("Y-m-d", strtotime($check_out));
    $check_out_less     = date("Y-m-d", strtotime("-1 day", strtotime($check_out)));
    ...
    if ($n_days != $available) {throw new Exception($NO_AVAILABILITY);}
    ...
    $values = [$first_name, $last_name, $phone, $email, $check_in, $check_out, $cc_name, $cc_number, $cc_expire, $cc_cvc, $iv_number, $iv_expire, $iv_cvc, $room_type, $double_bed, $ano, $comments, $new_booking_status, $date, $ip, $cancel_policy, $n_days, $av_price, $total_price];
    ...
    $update_arr = sqlite("QUERY_FETCH_ASSOC",
    ...
    $voucher  = sqlite("QUERY_FETCH_ASSOC",
...
HTML;
    ...
    mail($email, "$YOUR_BOOKING @ $MY_HOTEL ✔️", $message_guest, implode("\r\n", $headers));
    mail($booking_mail_from, "✔️ You Have Received a New Booking", $message_hotel, implode("\r\n", $headers));

    $response = '{"response": "ok", "message": "' . $YOUR_RESERVATION_CONFIRMED . '"}';
    header("Content-Type: application/json");
    echo $response;
    
    } catch (Exception $e) {

    $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>Warning! </strong>' . $e->getMessage() .' !<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><span aria-hidden="true"></span></button></div>';
  ...
  case "json_calendar":
    ...
    echo json_encode($select_calendar, JSON_PRETTY_PRINT, JSON_UNESCAPED_UNICODE);
  ...
  case "calendar_update":
    ...
    break;
  case "json_bookings":
    $select_bookings = sqlite("QUERY_FETCH_ASSOC",
    ...
    header("Content-Type: application/json; charset=utf-8");
    echo json_encode($select_bookings, JSON_PRETTY_PRINT, JSON_UNESCAPED_UNICODE);
  ...
  case "my_dencrypt":
    ...
    $select_cc = sqlite("QUERY_FETCH_ASSOC",
    ...
    $decode = my_decode(array_merge($array_NUMBER, $array_EXPIRE, $array_CVC));
    $decode = array_column($decode, 'decrypted_token');
    $cc_n   = chunk_split($decode[0], 4, ' '); unset($decode[0]);
...
try {
    ...
    mail("Contact Form <$booking_mail_from>", "✔️ New Message From Contact Form", $message, implode("\n", $headers));

    $response = '{"response": "ok", "message": "' . $CONTACT_FORM_SUCCESS . '"}';
    header("Content-Type: application/json");
    echo $response;
...
try {
    ...
    if (isset($_POST) && $user != "" && $password != "") {
        $login = sqlite("QUERY_FETCH_ASSOC",
        ...
        if ($password == $salted_hash) {
            ...
            if (!empty($_POST["remember"])) {
                setcookie("username", $_POST["user_email"], $cookie_params);
                setcookie("password", $_POST["password"], $cookie_params);
            } else {
                setcookie("username", "", $cookie_params);
                setcookie("password", "", $cookie_params);
            }

            $message  = "<strong>" . "authentication_successful" . "</strong>";
            $response = '{"response": "ok", "message": "' . $_COOKIE['origin_ref'] . '"}';
...
try {
    ...
    if (move_uploaded_file($tmp, $path)) {
        ...
        echo '<div id="err" class="alert alert-danger alert-dismissible fade show fw-bolder" role="alert">' . $e->getMessage().' <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
}
...
foreach (range(1, 7) as $number) {
    ...
    $values = ["New Img Alt $number", "New Img Description $number", "https://fakeimg.pl/500x300/0d6efd/?font=lobster&text=New Img Src $number", $gallerytype, $gallerylanguage, $galleryname,];
...
try {
  ...
  case "admin_gallery_add_image":
    ...
    $values         = [$imgdescription, $imgsrc, $imgalt, $imgtype, $imglanguage, $galleryname,];
...
try {
  ...
  case "admin_room_add_room":
    ...
    $values          = [$roomname, $roomdescription, $roomfacilities, $roommaxlotation, $roomimgsrc, $roomimgalt, $roomtype, $roomlanguage];
...
try {
  ...
  case "admin_service_add_service":
    ...
    $values             = [$servicename, $servicedescription, $serviceimgsrc, $serviceimgalt, $servicelanguage];
...
try {
  ...
  case "admin_save_template":
    ...
    $file     = $_POST["file"];
    $myfile   = fopen("../assets/html/$file", "w") or die("$UNABLE_OPEN_FILE!");
    $txt      = $template;
...
try {
    ...
    $year_exists = sqlite("QUERY_FETCH_ASSOC",
        "SELECT COUNT(DAY) AS COUNT FROM CALENDAR WHERE DAY LIKE '%$add_year-%';");

    if (empty($add_year)) { throw new Exception($INVALID_YEAR); }
    if ($year_exists[0]["COUNT"] != 0) { throw new Exception($YEAR_EXISTS); }
    ...
    foreach ($period as $day) {
        ...
        foreach ($room_types as $type) {
            $type = $type["TYPE"];
            $add_year = $_POST["add_year"];
            ${"add_availability_$type"} = $_POST["add_availability_$type"];
            ${"add_price_$type"} = $_POST["add_price_$type"];
            ${"add_status_$type"} = $_POST["add_status_$type"];

            $values = [ "$day", "$type", ${"add_availability_" . $type}, ${"add_price_" . $type}, ${"add_status_" . $type} ];

            $insert = sqlite("INSERT",
                "INSERT INTO CALENDAR (DAY,ROOM_TYPE,AVAILABILITY,PRICE,STATUS) VALUES (?,?,?,?,?);");

            if ($insert <= 0) { throw new Exception($ROW_INSERT_FAIL); }
        }
    }
...
*/
  ...
  case "json_language":
    ...
    $select_language = sqlite("QUERY_FETCH_ASSOC",
            "SELECT ID, NAME, TRANSLATION FROM 'LANGUAGE_$language';");

    header("Content-Type: application/json; charset=utf-8");
    echo json_encode($select_language, JSON_PRETTY_PRINT, JSON_UNESCAPED_UNICODE);
...
try {
    ...
    $user_array = sqlite("QUERY_FETCH_ASSOC",
    ...
    if ($cur_password_dec !== $cur_password) { throw new Exception($CURRENT_PASSWORD_INCORRECT); }
    if (empty($new_password)) { throw new Exception($EMPTY_PASSWORD_NOT_ALLOWED); }
    if ($new_password !== $confirm_password) { throw new Exception($NEW_PASSWORD_DOES_NOT_MATCH); }
    ...
    if ($update !== 1) { throw new Exception($UPDATE_FAIL); }

    $response = '{"response": "ok", "message": "' . $PASSWORD_CHANGED_SUCCESSFULLY . '"}';
...
try {
    ...
    $response = '{"response": "ok", "message": "' . 'Language Changed' . '"}';
